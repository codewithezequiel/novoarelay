...
export default function TRListItem({ report }) {
  return (
    <View className="mb-4 rounded-xl bg-white p-5 shadow-md">
      {/* Timestamp */}
      ...
      <View className="mt-3 flex-row items-center justify-between">
        ...
        <View className="flex-row items-center gap-3">
          <Image source={{ uri: report.employee_image_url }} className="h-10 w-10 rounded-full" />
          <View>
            <Text className="font-semibold">{report.employee_fullName || 'Unknown Employee'}</Text>
